...
@implementation ZMShopScrollView
...
- (void)initView
{
    ...
    _pageControl.currentPageIndicatorTintColor = RGBCOLOR(245, 50, 109);
...
-(void)setFromeString:(NSString *)fromeString
{
    ...
    {
        [NSTimer scheduledTimerWithTimeInterval:2 target:self selector:@selector(keepMove) userInfo:nil repeats:YES];
        _num=1;
    }
}
-(void)keepMove
...
- (void)setImageArry:(NSArray *)imageArry
{
    ...
    for (int i = 0; i < imageArry.count; i++) {
        UIImageView *image = [[UIImageView alloc]initWithFrame:CGRectMake( APP_SCREEN_WIDTH * i , 0, imageHeight, imageHeight)];
        image.tag = 11 + i;
